...
Request.prototype._initPOST = function (opts) {
  ...
  self.URL = StringUtils.isAbsoluteURL(opts.endpoint) ? opts.endpoint : urljoin(constants.core.baseURL, opts.endpoint)
...
Request.prototype.start = function (cb) {
  ...
  function wrapcb (err, response, body) {
    ...
    if (contentType && contentType !== 'application/octet-stream') {
      response.body = body = iconv.decode(response.body, 'utf8')
    }
    ...
    if (err) {
      ...
      debug('Snapchat Request Error: %d (%s) \nendpoint: %s \nheaders: %j \nrequest: %s',
            ...
            self.multipart ? '' : (response.request.body ? response.request.body.toString('utf-8').substr(0, 80) : ''))
