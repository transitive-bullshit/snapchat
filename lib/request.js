...
Request.prototype.start = function (cb) {
  ...
  function wrapcb (err, response, body) {
    if (err) {
      ...
      debug('Snapchat Request Error: %d (%s) (request %s)',
            response.statusCode,
      ...
      cb('Snapchat API error ' + response.statusCode + ' (' + response.statusMessage + ')', body)
    } else {
 ...
 */
Request.post = function (endpoint, params, gauth, token, cb) {
  ...
  return Request.postCustom(endpoint, params, headers, token, cb)
}
 ...
 * @return {Request} new Request wrapping this HTTP POST request.
 */
Request.postCustom = function (endpoint, params, headers, token, cb) {
  ...
  request.post(extend(params, {
    ...
    if (encoding === 'gzip') {
      zlib.gunzip(body, function(err, dezipped) {
        cb(null, dezipped && dezipped.toString())
      })
    } else if (encoding === 'deflate') {
      zlib.inflate(body, function(err, decoded) {
        cb(null, decoded && decoded.toString())
      })
    } else {
      cb(null, body)
    }
