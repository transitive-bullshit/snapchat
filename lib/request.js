...
Request.prototype.start = function (cb) {
  ...
  function wrapcb (err, response, body) {
    ...
    if (!err && response && (response.statusCode < 200 || response.statusCode >= 300)) {
      ...
      cb('Snapchat API error ' + response.statusCode + ' (' + response.statusMessage + ')', response, body)
    } else {
  ...
  if (self.HTTPMethod === 'POST') {
    ...
    debug('%s Request %s (%j) \'%s\'', self.HTTPMethod, self.URL, self.HTTPHeaders,
    ...
    debug('%s Request %s (%j)', self.HTTPMethod, self.URL, self.HTTPHeaders)
  }
}
